...
pub fn mark_transaction(transaction : &String, flag: &str){
    ...
    let mut read_bytes = recovery_reader.read_line(&mut line).expect("Cannot read file");
    while read_bytes > 0 {
        ...
        if id == id_target{
            recovery_writer.seek(SeekFrom::Start(seek-3)).expect("should move");
        ...
        line.clear();
        read_bytes = recovery_reader.read_line(&mut line).expect("Cannot read file");
    }

    let _ = recovery_writer.seek(SeekFrom::End(0));
    write!(recovery_writer,"{},{}\n",transaction,flag).expect("Error al grabar inicio de transaction");
    recovery_writer.flush().unwrap();
