...
impl LeaderElection {
    ...
    fn find_new(&mut self) {
        ...
        *self.leader_id.0.lock().unwrap() = None;
        ...
        let got_ok =
            self.got_ok
                .1
                .wait_timeout_while(self.got_ok.0.lock().unwrap(), TTL_LEADER, |got_it| !*got_it);
        if !*got_ok.unwrap().0 {
            self.make_me_leader()
        } else {
            let _guard = self
    ...
    fn make_me_leader(&mut self) {
        ...
        self.logger.log(
            format!("[{}] I am your new master", self.id).as_str(),
            STATUS_INFO,
        );
    ...
    fn responder(&mut self) {
        while !*self.stop.0.lock().unwrap() {
            ...
            match &buf[0] {
                b'O' => {
                    self.logger.log(
                        format!("Received ok from {}", id_from).as_str(),
                        STATUS_INFO,
                    );
                ...
                b'E' => {
                    self.logger.change_id_path(self.id);
                    self.logger.log(
                        format!("received election from {}", id_from).as_str(),
                        STATUS_INFO,
                    );
                    if id_from < self.id {
                        self.socket
                ...
                b'C' => {
                    self.logger.log(
                        format!("Received new coordinator from {}", id_from).as_str(),
                        STATUS_INFO,
                    );
                    if id_from < self.id {
                        self.socket
                ...
                _ => {
                    self.logger.log(
                        format!(
                            "received weird stuff from {}, starting message {}",
                            from, id_from
                        )
                        .as_str(),
                        STATUS_ERROR,
...
fn team_member(mut leader: LeaderElection, mut logger: Logger) {
    ...
    loop {
        if leader.am_i_leader() {
            ...
            socket
                .set_read_timeout(Some(Duration::from_secs(READ_TIMEOUT_CHECK)))
                .unwrap();
            ...
            socket
                .send_to("Heart".as_bytes(), id_to_dataaddr(leader_id))
                .unwrap();
            ...
            if let Ok((_size, _from)) = socket.recv_from(&mut buf) {
                ...
                logger.log(
                    "something happened to leader, checking election",
                    STATUS_ERROR,
                );
                leader.find_new()
            }
