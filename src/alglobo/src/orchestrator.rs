...
pub fn orchestrate(
    ...
    let (id, amount_air, amount_bank, amount_hotel) = (
        v[0].to_owned(),
    ...
    if amount_hotel != 0 {
        ...
        v.push(thread::spawn(move || {
            send_req(HOTEL_ADDR.to_owned(), amount_hotel, b, f, i)
        }));
    }
    if amount_bank != 0 {
        ...
        let i = id.clone();
        v.push(thread::spawn(move || {
            send_req(BANK_ADDR.to_owned(), amount_bank, b, f, i)
        }));
    }
    if amount_air != 0 {
        ...
        let i = id.clone();
        v.push(thread::spawn(move || {
            send_req(AER_ADDR.to_owned(), amount_air, b, f, i)
        }));
    }
    ...
    if let Ok(f) = flag.read() {
        if !*f {
            ...
            logger.log(
                &format!("Transaction {} FAILED. Sending to Deadletter", id),
                "ERROR",
            );
