use std::{
    fs::File,
    io::{self, stdin, BufRead, BufReader, BufWriter, Seek, SeekFrom, Write},
    sync::{Arc, RwLock},
};
use std_semaphore::Semaphore;

use crate::{logger::Logger, orchestrator};

const DEADLETTER_FILE: &str = "dead_letter.txt";

pub(crate) fn new_dead(transaction: String) {
    let deadletter = File::options()
        .append(false)
        .read(true)
        .write(true)
        .create(true)
        .open(DEADLETTER_FILE);
...
pub(crate) fn read_dead_letter() {
    ...
    if let Ok(ref file) = deadletter {
        ...
        logger.log(
            format!("Error opening {}", DEADLETTER_FILE).as_str(),
            "ERROR",
        );
        return;
    }
    ...
    while read_bytes > 0 && !skipped {
        ...
        if state == "F" {
            print!(
                "Transacción with state {} => {} | ",
                state, dead_transaction
            );
            ...
            loop {
                ...
                match (&input_string.to_lowercase()).as_str().trim_end() {
                    ...
                    &_ => {
                        println!(
                            "{} is not valid. Only P/N/R/E keys are valid.. ",
                            (&input_string.to_lowercase()).as_str().trim_end()
                        );
                    }
                }
            }
