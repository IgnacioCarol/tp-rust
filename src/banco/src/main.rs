...
impl Handler<Add> for Banco {
    ...
    fn handle(&mut self, msg: Add, _ctx: &mut Self::Context) -> Self::Result {
        self.amount += msg.0;
        self.logger.do_send(Log((format!("[BANCO ACTOR] Amount changed, current is: {}", self.amount), STATUS_INFO.to_string()))).expect("should be sent");
        self.amount
    }
}
...
impl BancoSocket {
    fn new(banco_actor: Recipient<Add>, logger_actor: Recipient<Log>, message_sent: Arc<Semaphore>) -> BancoSocket {
        BancoSocket {
...
async fn main() {
    ...
    let actor_banco = Banco {amount: 0, logger: l_1}.start();
